    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            setOverlayContent(
                ...
                'Microsoft 인증에 실패했습니다. 다시 시도하십시오.',
                '확인'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    '경고<br>로그아웃 하시겠습니까?',
                    '서버 및 런처를 이용 하시려면 로그인이 필요합니다!<br><br>마인크래프트 정품을 가지고 계신 Microsoft<br>계정이 필요합니다.',
                    '로그아웃',
                    '취소'
                )
                setOverlayHandler(() => {
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGOUT, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            setOverlayContent(
                'Something Went Wrong',
                'Microsoft 로그아웃에 실패했습니다. 다시 시도하십시오.',
                '확인'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        ...
        titleElement.innerHTML = '안정화된 버전'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/eveooline1538/EvePixelmon/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = '릴리스 노트를 로드하지 못했습니다.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        settingsUpdateTitle.innerHTML = '최신 버전을 실행 중입니다.'
